...
/**
 * One coherent V8 world. Each reconstructed sector owns its own geometry.
 * The warehouse is the first premium vertical slice and replaces the original
 * primitive warehouse entirely.
 */
export class World {
  ...
  private buildWarehouse(): void {
    ...
    for (const [id, item] of this.warehouse.carryables) this.carryables.set(id, item);
    for (const [id, socket] of this.warehouse.sockets) this.sockets.set(id, socket);
  }
  ...
  private buildCapa(): void {
    ...
    const beacon = new THREE.Mesh(
      new THREE.CylinderGeometry(0.6, 0.8, 3.6, 12),
      new THREE.MeshStandardMaterial({ color: C.purple, emissive: C.purple, emissiveIntensity: 0.16, roughness: 0.42 })
    );
  ...
  private zoneBuilding(id: ZoneId, x: number, z: number, w: number, d: number, accent: number, title: string): void {
    const floor = this.meshBox(w, 0.12, d, 0x45535a);
    ...
    const back = new THREE.Mesh(new THREE.BoxGeometry(w, 3.1, 0.35), wallMat);
    back.position.set(x, 1.55, z - d / 2);
    back.castShadow = true;
    this.group.add(back);
    const left = new THREE.Mesh(new THREE.BoxGeometry(0.35, 3.1, d), wallMat);
    left.position.set(x - w / 2, 1.55, z);
    left.castShadow = true;
    this.group.add(left);
    const right = left.clone();
    right.position.x = x + w / 2;
    this.group.add(right);
    const header = new THREE.Mesh(new THREE.BoxGeometry(w, 0.22, 0.45), accentMat);
    header.position.set(x, 3.05, z - d / 2 + 0.12);
  ...
  private worker(x: number, z: number, accent: number): THREE.Group {
    ...
    const legs = [-0.19, 0.19].map((dx) => {
      const m = new THREE.Mesh(new THREE.BoxGeometry(0.26, 0.85, 0.3), pants);
      m.position.set(dx, 0.65, 0);
      return m;
    });
    g.add(torso, head, helmet, ...legs);
    g.traverse((n) => { if (n instanceof THREE.Mesh) n.castShadow = true; });
    this.group.add(g);
    return g;
  }
  ...
  private terminal(x: number, z: number, accent: number, label: string): THREE.Group {
    const g = new THREE.Group();
    g.position.set(x, 0, z);
    const base = new THREE.Mesh(new THREE.BoxGeometry(1.05, 1.25, 0.72), new THREE.MeshStandardMaterial({ color: C.dark, roughness: 0.6, metalness: 0.25 }));
    base.position.y = 0.63;
    base.castShadow = true;
    const screen = new THREE.Mesh(new THREE.BoxGeometry(0.78, 0.42, 0.04), new THREE.MeshStandardMaterial({ color: accent, emissive: accent, emissiveIntensity: 0.2, roughness: 0.38 }));
    screen.position.set(0, 0.82, 0.38);
    g.add(base, screen);
    this.addLocalLabel(g, label, 1.45, accent);
    this.group.add(g);
    return g;
  }
  ...
  private gaugeStation(x: number, z: number, index: number): THREE.Group {
    const g = new THREE.Group();
    g.position.set(x, 0, z);
    const bench = new THREE.Mesh(new THREE.BoxGeometry(3.2, 0.82, 1.65), new THREE.MeshStandardMaterial({ color: 0x65747b, roughness: 0.7, metalness: 0.2 }));
    bench.position.y = 0.42;
    bench.castShadow = true;
    const gauge = new THREE.Mesh(new THREE.CylinderGeometry(0.36, 0.36, 0.16, 18), new THREE.MeshStandardMaterial({ color: 0xe5ecee, roughness: 0.4 }));
    gauge.rotation.x = Math.PI / 2;
    gauge.position.set(0, 1.12, 0);
    g.add(bench, gauge);
    this.addLocalLabel(g, `M-0${index}`, 1.85, 0x5287b7);
    this.group.add(g);
    return g;
  }
  ...
  private machine(x: number, z: number): THREE.Group {
    const g = new THREE.Group();
    g.position.set(x, 0, z);
    const body = new THREE.Mesh(new THREE.BoxGeometry(5.5, 2.5, 3.6), new THREE.MeshStandardMaterial({ color: 0x53646c, roughness: 0.58, metalness: 0.28 }));
    body.position.y = 1.25;
    body.castShadow = true;
    const rotor = new THREE.Mesh(new THREE.CylinderGeometry(0.82, 0.82, 0.26, 14), new THREE.MeshStandardMaterial({ color: C.orange, roughness: 0.45, metalness: 0.2 }));
    rotor.rotation.x = Math.PI / 2;
    rotor.position.set(0, 1.6, 1.95);
  ...
  private packageBox(x: number, z: number, label: string): THREE.Group {
    const g = new THREE.Group();
    g.position.set(x, 0, z);
    const b = new THREE.Mesh(new THREE.BoxGeometry(1.65, 1.15, 1.25), new THREE.MeshStandardMaterial({ color: 0x9a7147, roughness: 0.86 }));
    b.position.y = 0.58;
    b.castShadow = true;
    g.add(b);
    this.addLocalLabel(g, label, 1.5, 0xb77042);
    this.group.add(g);
    return g;
  }
  ...
  private socketPad(x: number, z: number, color: number, label: string): THREE.Group {
    const g = new THREE.Group();
    g.position.set(x, 0, z);
    const pad = new THREE.Mesh(new THREE.CylinderGeometry(1.4, 1.4, 0.08, 20), new THREE.MeshStandardMaterial({ color, emissive: color, emissiveIntensity: 0.08, roughness: 0.65 }));
    pad.position.y = 0.05;
    g.add(pad);
    this.addLocalLabel(g, label, 0.42, color);
    this.group.add(g);
    return g;
  }

  private addConveyor(x: number, z: number, length: number): void {
    const frame = new THREE.Mesh(new THREE.BoxGeometry(length, 0.75, 2.0), new THREE.MeshStandardMaterial({ color: 0x3f5059, roughness: 0.55, metalness: 0.3 }));
    frame.position.set(x, 0.52, z);
    frame.castShadow = true;
    this.group.add(frame);
    const rollerMat = new THREE.MeshStandardMaterial({ color: 0xaab3b6, roughness: 0.32, metalness: 0.7 });
    for (let i = 0; i < 12; i++) {
      const r = new THREE.Mesh(new THREE.CylinderGeometry(0.11, 0.11, 1.7, 8), rollerMat);
      r.rotation.x = Math.PI / 2;
      r.position.set(x - length / 2 + 1 + i * ((length - 2) / 11), 0.96, z);
      this.group.add(r);
    }
  }

  private action(id: string, prompt: string, object: THREE.Object3D, radius: number): void {
    const marker = new THREE.Mesh(new THREE.TorusGeometry(0.24, 0.045, 8, 20), new THREE.MeshBasicMaterial({ color: C.yellow }));
  ...
  private addLocalLabel(parent: THREE.Object3D, text: string, y: number, color: number): void {
    const sprite = this.labelSprite(text, color, 2.7);
  ...
  private labelSprite(text: string, color: number, width: number): THREE.Sprite {
    ...
    ctx.fillStyle = 'rgba(8,18,24,.86)';
    ctx.roundRect(6, 10, 500, 108, 18);
    ctx.fill();
  ...
  private meshBox(w: number, h: number, d: number, color: number): THREE.Mesh {
